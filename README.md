...
Подготовлен стартовый каркас и реализован воспроизводимый генератор независимых Reference/Current-выборок со сценариями числового, категориального дрейфа и роста пропусков. Статистические методы, проверки и ML-модуль пока остаются явными заглушками с `NotImplementedError`. Notebook является шаблоном, а Streamlit показывает стартовую страницу. Фактических результатов экспериментов пока нет.
...
Сейчас добавлены тесты генератора данных; проверки остальных модулей появляются вместе с их реализацией. План проверок находится в [tests/README.md](tests/README.md).
