...
Реализация построена поверх принятого change set `636fbe4`. Точный SHA merge
commit в `main` нужно записать после успешного `git fetch origin`: он не
выводится из SHA исходной ветки и поэтому здесь не выдумывается.
